...
export default function ServicesPage() {
  ...
  // In Next.js App Router, the param name matches the folder name [dentist_slug]
  const tenant = params.dentist_slug as string
  ...
  useEffect(() => {
    if (initialized && (!user || user.role !== 'dentist')) {
      const tenantSlug = tenant || config?.slug || 'lumina'
      router.replace(`/${tenantSlug}/login`)
    }
  }, [initialized, user, tenant, config, router])
