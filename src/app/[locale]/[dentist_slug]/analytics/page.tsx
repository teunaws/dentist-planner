...
// Dynamically import Recharts component with no SSR to avoid hydration issues
const AnalyticsChart = dynamic(
  () => import('../../../../components/dentist/AnalyticsChart'),
  { ssr: false }
)
...
export default function AnalyticsDashboard() {
  ...
  const { slug, analyticsData, isAnalyticsLoading } = useTenant()
  ...
  useEffect(() => {
    console.log('[AnalyticsDashboard] Render:', {
      ...
      dataLength: analyticsData?.newVsReturning?.length
    })
  ...
  useEffect(() => {
    if (user && user.role !== 'dentist') {
      const tenantSlug = slug || (tenant as string) || 'lumina'
      router.replace(`/${tenantSlug}/login`)
    }
  }, [user, slug, tenant, router])

  if (!user || user.role !== 'dentist') {
    // Redirect logic moved to useEffect below
    return null
  }
        ...
        <div className="grid flex-1 gap-4 overflow-hidden lg:grid-cols-2">
          ...
          <GlassCard className="rounded-3xl p-6 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 min-h-[400px]">
              <AnalyticsChart chartData={chartData} yAxisMax={yAxisMax} />
            </div>
          </GlassCard>
